...
app.use((req, res, next) => {
	res.set('Access-Control-Allow-Origin', 'https://rvr.secde.fun'); // this is the rocket.chat URL
	...
	if (!currentUsername) {
		axios.post('https://rvr.secde.fun/api/v1/users.register', {
			...
			if (response.data.success) {
				return axios.post('https://rvr.secde.fun/api/v1/login', {
					username: 'new-user',
		...
		axios.post('https://rvr.secde.fun/api/v1/login', {
		...
		axios.post('https://rvr.secde.fun/api/v1/users.register', {
			...
			if (response.data.status === 'success') {
				...
				window.parent.postMessage({
					...
					loginToken: '${ response.data.data.authToken }'
				}, 'https://rvr.secde.fun'); // rocket.chat's URL
				</script>`);
			}
		}).catch(function (error) {
			res.sendStatus(401);
